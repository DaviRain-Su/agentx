...
**核心叙事（黑客松 Pitch）：**
> "你的量化策略值钱但不能开源。Gradience 让你把它变成一个 Agent 节点，在本地运行，通过 x402 自动收费，代码永远不离开你的机器。"
...
**Worker URL（已部署后）：** `https://gradience-worker.davirain-yin.workers.dev`
...
| Worker 部署上线 | `https://gradience-worker.davirain-yin.workers.dev` | ✅ |
...
| agent-sdk：createGradienceTools（7个工具）+ createGradienceSession | `packages/agent-sdk/src/` | ✅ |
...
Worker URL：`https://gradience-worker.davirain-yin.workers.dev`
...
**index.ts 核心逻辑：**
```typescript
import { createGradienceTools, AgentRegistryService, deriveAgentAddress } from "@gradience/agent-sdk";
