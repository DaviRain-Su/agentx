...
│   ├── worker-cloudflare/      # AgentX 自己的 demo Worker（也是参考实现）
...
AgentX 使用**确定性钱包派生**，一个 masterKey 可以派生出多个 agent 钱包：
...
开发者用这个 SDK 把自己的 Agent 接入 AgentX 网络。
...
```typescript
import { createAgentXTools } from "@agentx/agent-sdk";

const tools = createAgentXTools({
...
**这是 AgentX 自己运行的 demo Worker**，同时也是开发者的参考实现模板。
...
8. **开发者如何发布自己的 Agent 到 AgentX Market**：
...
```bash
curl https://agentx-worker.xxx.workers.dev/health
curl https://agentx-worker.xxx.workers.dev/api/agents
curl -X POST https://agentx-worker.xxx.workers.dev/api/a2a/simulate \
